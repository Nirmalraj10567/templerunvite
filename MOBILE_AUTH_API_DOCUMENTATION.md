...
1. **Send OTP**
```bash
curl -X POST http://localhost:4000e-auth/send-otp \
...
2. **Verify OTP**
```bash
curl -X POST http://localhost:4000e-auth/verify-otp \
  -H "Content-Type: application/json" \
  -d '{
    "mobileNumber": "9999999999",
    "name": "John Doe",
...
3. **Get Payment Details**
```bash
curl -X GET http://localhost:4000e-auth/payment-details/1 \
