...
async function testAnnadhanamLogs() {
  ...
  try {
    const response = await fetch('http://localhost:4000hanam/logs', {
      headers: {
  ...
  try {
    const response = await fetch('http://localhost:4000hanam/1/logs', {
      headers: {
        'Authorization': 'Bearer your-test-token-here' // Replace with actual token
      }
    });
  ...
  try {
    const response = await fetch('http://localhost:4000hanam', {
      method: 'POST',
      headers: {
    ...
    if (result.success && result.data?.id) {
      ...
      const logsResponse = await fetch(`http://localhost:4000hanam/${result.data.id}/logs`, {
        headers: {
          'Authorization': 'Bearer your-test-token-here' // Replace with actual token
        }
      });
