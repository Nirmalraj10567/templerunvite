...
```bash
curl -X GET "http://localhost:4000hanam/123/logs" \
  -H "Authorization: Bearer your-token"
```

### Get All Logs with Pagination
```bash
curl -X GET "http://localhost:4000hanam/logs?page=1&pageSize=20" \
  -H "Authorization: Bearer your-token"
```
...
curl -X POST http://localhost:4000hanam \
...
curl -X GET "http://localhost:4000hanam/logs" \
  -H "Authorization: Bearer your-token"
```
