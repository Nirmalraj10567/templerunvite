...
export default function MasterAdminPage() {
  ...
  const loadStats = async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/superadmin/tenants/stats', { headers: { Authorization: `Bearer ${token}` }});
  ...
  const setYearEndStatus = async (patch: Partial<{ enforced: boolean; locked: boolean }>) => {
    await fetch('/api/system/year-end-status', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
      body: JSON.stringify(patch),
    });
    const r = await fetch('/api/system/year-end-status', { headers: { Authorization: `Bearer ${token}` }});
    const d = await r.json();
    setYearEnd(d?.data || { enforced: false, locked: false });
  };

  const refreshTenants = async () => {
    const r = await fetch('/api/superadmin/tenants', { headers: { Authorization: `Bearer ${token}` }});
    const d = await r.json();
    setTenants(d?.data || []);
  };
  ...
  const onDelete = async (t: Tenant) => {
    ...
    await fetch(`/api/superadmin/tenants/${t.id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${token}` }});
    await refreshTenants();
  };

  const checkHealth = async (t: Tenant) => {
    const r = await fetch(`/api/superadmin/tenants/${t.id}/health`, { headers: { Authorization: `Bearer ${token}` }});
    const d = await r.json();
