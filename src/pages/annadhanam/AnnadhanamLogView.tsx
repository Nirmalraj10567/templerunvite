...
export default function AnnadhanamLogView({ recentOnly = false }: AnnadhanamLogViewProps) {
                  ...
                  {filteredLogs.length === 0 ? (
                    ...
                    filteredLogs.map((lg) => (
                      <TableRow key={lg.id}>
                        ...
                        <TableCell>{lg.annadhanam_id}</TableCell>
                        <TableCell>{lg.annadhanam_name ?? '-'}</TableCell>
                        <TableCell>{lg.receipt_number ?? '-'}</TableCell>
                        <TableCell>
                          {(() => {
          ...
          <div className="flex items-center justify-between py-4">
            <div className="text-sm text-muted-foreground">
              ...
              <span className="font-medium">
                {filteredLogs.length > 0 
                  ? `${(pagination.page - 1) * pagination.pageSize + 1}–${Math.min(pagination.page * pagination.pageSize, filteredLogs.length)}` 
                  : '0'}
              </span>{" "}
              {t('of', 'இல்')}{" "}
