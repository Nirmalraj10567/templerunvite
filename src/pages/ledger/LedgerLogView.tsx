...
export default function LedgerLogView({ recentOnly = false }: LedgerLogViewProps) {
  ...
  const fetchLogs = useCallback(async (page = 1, search = "") => {
    ...
    try {
      ...
      const res = await fetch(
        `https://tmsapi.xesstechlink.com/api/ledger-entries/logs?${params}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{t('Entry ID', 'உள்ளீடு ஐடி')}</TableHead>
              ...
              {logs.length === 0 ? (
                ...
                logs.map((lg) => (
                  <TableRow key={lg.id}>
                    ...
                    <TableCell>{lg.ledger_entry_id}</TableCell>
                    <TableCell>{lg.entry_name ?? '-'}</TableCell>
                    <TableCell>
                      {(() => {
