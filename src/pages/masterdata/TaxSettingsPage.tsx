...
export default function TaxSettingsPage() {
  ...
  const loadSettings = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('https://templeapi.agniplay.com/api/tax-settings', {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = editingId 
        ? `https://templeapi.agniplay.com/api/tax-settings/${editingId}`
        : 'https://templeapi.agniplay.com/api/tax-settings';
  ...
  const handleDelete = async (id: number, year: number) => {
    ...
    try {
      const response = await fetch(`https://templeapi.agniplay.com/api/tax-settings/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleBulkToggle = async (enableOutstanding: boolean) => {
    ...
    try {
      const response = await fetch('https://templeapi.agniplay.com/api/tax-settings/bulk-toggle', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`
