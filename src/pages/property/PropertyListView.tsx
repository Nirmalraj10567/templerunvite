...
export default function PropertyListView() {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      // Build a robust merged object for UI update
      const mergedLocal: Property = {
        ...viewEditProperty,
        name: editedProperty.name ?? viewEditProperty.name,
        details: editedProperty.details ?? viewEditProperty.details,
        value: editedProperty.value ?? viewEditProperty.value,
        // Prefer server timestamps/fields when available
        ...(updatedProperty || {}),
      } as Property;

      setData((prev) => prev.map((prop) => (prop.id === viewEditProperty.id ? mergedLocal : prop)));
                  ...
                  {allColumns.map(
                    (col) =>
                      visibleCols[col.key] && (
                        <th
                          key={col.key}
                          className={`px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider ${col.align === 'right'
                              ? 'text-right'
                              : col.align === 'center'
                                ? 'text-center'
                                : 'text-left'
