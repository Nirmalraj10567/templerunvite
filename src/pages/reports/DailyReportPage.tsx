...
export default function DailyReportPage() {
  ...
  const fetchReport = async (d: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`https://templeapi.agniplay.com/api/reports/daily?date=${encodeURIComponent(d)}`, {
  ...
  const resetReport = async () => {
    ...
    try {
      const res = await fetch(`https://templeapi.agniplay.com/api/reports/daily?date=${encodeURIComponent(date)}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
