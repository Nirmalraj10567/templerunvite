...
module.exports = function(deps = {}) {
  ...
  /**
   * Mobile API - Get calendar data for a specific date
   * GET /api/mobile/calendar/:date
  ...
  router.get('/:date', async (req, res) => {
    try {
      ...
      // Format the response for mobile (simplified structure)
      const mobileCalendarData = {
        date,
      ...
      res.json(mobileCalendarData);

    } catch (error) {
      console.error('Mobile calendar API error:', error);
      res.status(500).json({ 
  ...
  /**
   * Mobile API - Get calendar data for today
   * GET /api/mobile/calendar/today
   */
  router.get('/today', async (req, res) => {
    try {
      ...
      // Fetch approved pooja for today
      const pooja = await db('pooja')
        .select('id', 'name', 'time', 'from_date', 'status')
        .where('temple_id', templeId)
        .andWhere('status', 'approved')
      ...
      // Fetch approved hall bookings for today
      const hallBookings = await db('marriage_hall_bookings')
        .select('id', 'event', 'name', 'check_in_date', 'check_in_time', 'status')
        .where('temple_id', templeId)
        .andWhere('status', 'approved')
        .andWhere('check_in_date', today)
        .orderBy('check_in_time', 'asc');
      ...
      console.error('Mobile calendar today API error:', error);
      res.status(500).json({ 
        error: 'Failed to fetch today\'s calendar data',
        details: error.message 
      });
    }
  });
  ...
  /**
   * Mobile API - Get calendar data for a date range
   * GET /api/mobile/calendar/range?from=YYYY-MM-DD&to=YYYY-MM-DD
   */
  router.get('/range', async (req, res) => {
    try {
      ...
      console.error('Mobile calendar range API error:', error);
      res.status(500).json({ 
        error: 'Failed to fetch calendar range data',
        details: error.message 
      });
    }
  });
  ...
  /**
   * Mobile API - Get all calendar data (global view)
   * GET /api/mobile/calendar/all
   */
  router.get('/all', async (req, res) => {
    try {
      ...
      console.error('Mobile calendar all API error:', error);
      res.status(500).json({ 
        error: 'Failed to fetch all calendar data',
        details: error.message 
      });
    }
  });
