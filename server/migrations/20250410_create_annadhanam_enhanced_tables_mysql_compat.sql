-- Migration: Enhanced Annadhanam Tables (MySQL 5.7+ Compatible)
...
CREATE TABLE IF NOT EXISTS annadhanam_feedback (
  ...
  CONSTRAINT chk_food_rating CHECK (food_quality_rating >= 1 AND food_quality_rating <= 5),
  CONSTRAINT chk_service_rating CHECK (service_rating >= 1 AND service_rating <= 5),
  CONSTRAINT chk_cleanliness_rating CHECK (cleanliness_rating >= 1 AND cleanliness_rating <= 5)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
-- Using procedure to safely add columns if they don't exist
DELIMITER //

CREATE PROCEDURE IF NOT EXISTS AddAnnadhanamColumns()
BEGIN
  -- Add donor_id if not exists
  IF NOT EXISTS (
    ...
    ALTER TABLE annadhanam ADD COLUMN donor_id INT;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE annadhanam ADD COLUMN package_id INT;
  ...
  -- Add confirmation_status if not exists
  IF NOT EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'annadhanam' 
    ...
    ALTER TABLE annadhanam ADD COLUMN confirmation_status VARCHAR(50) DEFAULT 'pending';
    CREATE INDEX idx_annadhanam_confirmation ON annadhanam(confirmation_status);
  END IF;

  -- Add service_status if not exists
  IF NOT EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'annadhanam' 
    ...
    ALTER TABLE annadhanam ADD COLUMN service_status VARCHAR(50) DEFAULT 'pending';
  ...
  -- Add booking_source if not exists
  IF NOT EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'annadhanam' 
    ...
    ALTER TABLE annadhanam ADD COLUMN booking_source VARCHAR(50) DEFAULT 'web';
    CREATE INDEX idx_annadhanam_booking_source ON annadhanam(booking_source);
  END IF;

  -- Add is_walk_in if not exists
  IF NOT EXISTS (
    SELECT 1 FROM INFORMATION_SCHEMA.COLUMNS 
    WHERE TABLE_SCHEMA = DATABASE() 
    AND TABLE_NAME = 'annadhanam' 
    ...
    ALTER TABLE annadhanam ADD COLUMN is_walk_in TINYINT(1) DEFAULT 0;
  END IF;
END //
...
CALL AddAnnadhanamColumns();

-- Drop the procedure
DROP PROCEDURE IF EXISTS AddAnnadhanamColumns;
