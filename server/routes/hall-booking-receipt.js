...
module.exports = function createHallBookingReceiptRouter({ db, verifyQueryToken }) {
  ...
  router.get('/api/hall-bookings/:id/receipt.pdf', verifyQueryToken, async (req, res) => {
    try {
      ...
      const subHeader = settings?.hall_subheader || settings?.subheader || 'மண்டப முன்பதிவு ரசீது';
      ...
      // Meta row w/ centered subheader box
      ...
      label('ரசீது எண்', r.register_no || String(r.id), leftX);
      label('தேதி', r.date || '', rightX);
      ...
      const person = (r.name || '-').toString();
      const phone = (r.mobile || '-').toString();
      const sentence = `மண்டப முன்பதிவு: ${person} அவர்களிடமிருந்து நிகழ்ச்சி "${r.event || '-'}" க்காக முன்பதிவு செய்யப்பட்டது.`;
      ...
      doc.font(F_REG).fontSize(12).text(`முன்பணம்: ₹${toNum(r.advance_amount).toLocaleString('en-IN')}`, amountBoxX + 8, amountBoxY + 6);
      doc.font(F_REG).fontSize(12).text(`மொத்தம்: ₹${toNum(r.total_amount).toLocaleString('en-IN')}`, amountBoxX + 8, amountBoxY + 22);
      doc.font(F_REG).fontSize(12).text(`நிலுவை: ₹${toNum(r.balance_amount).toLocaleString('en-IN')}`, amountBoxX + 8, amountBoxY + 38);
