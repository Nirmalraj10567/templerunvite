...
module.exports = function(deps = {}) {
  ...
  async function ensurePaymentColumns() {
    try {
      const hasTable = await db.schema.hasTable('pooja');
      if (!hasTable) return;
      const hasPaymentMode = await db.schema.hasColumn('pooja', 'payment_mode');
      if (!hasPaymentMode) {
        await db.schema.alterTable('pooja', (t) => t.string('payment_mode', 20).nullable());
      }
      const hasAccountId = await db.schema.hasColumn('pooja', 'account_id');
      if (!hasAccountId) {
        await db.schema.alterTable('pooja', (t) => t.integer('account_id').nullable().index());
      }
    } catch (e) {
      // best-effort
      console.warn('ensurePaymentColumns skipped:', e.message);
    }
  }
  ...
  router.get('/', async (req, res) => {
    try {
      ...
      if (from) {
        query = query.where('pooja.booking_date', '>=', from);
      }

      if (to) {
        query = query.where('pooja.booking_date', '<=', to);
      }
  ...
  router.get('/bookings', async (req, res) => {
    try {
      ...
      const allBookings = await db('pooja')
        ...
        .select('id', 'receipt_number', 'name', 'from_date', 'to_date', 'time', 'booking_date')
      ...
      const bookings = allBookings.filter(booking => {
        const bd = booking.booking_date || booking.from_date;
        const bdt = new Date(bd);
        return bdt >= monthStart && bdt <= monthEnd;
      });
  ...
  router.post('/', async (req, res) => {
    try {
      ...
      const missing = [];
      if (!p.name && !p.userName) missing.push('name');
      if (!p.mobileNumber) missing.push('mobileNumber');
      if (!p.time) missing.push('time');
      if (!p.fromDate && !p.bookingDate) missing.push('bookingDate');
      ...
      // Ensure name is set (support both name and userName for compatibility)
      if (!p.name && p.userName) p.name = p.userName;


      const bookingDate = p.bookingDate || p.fromDate;
      const entryDate = p.entryDate || new Date().toISOString().slice(0, 10);
      let accountSel = null;
      try {
      ...
      if (!settings.multi_pooja_same_day) {
        const existingOnDate = await db('pooja')
          ...
          .where(builder => builder.where('booking_date', bookingDate).orWhere('from_date', bookingDate))
          .first();
      ...
      const record = {
        ...
        time: p.time,
        from_date: bookingDate,
        to_date: bookingDate,
        booking_date: bookingDate,
        entry_date: entryDate,
        remarks: p.remarks || null,
        transfer_to_account: 'POOJA INCOME A/C',
      ...
      try {
        ...
        if (hasJournal && !isNaN(amountNum) && amountNum > 0) {
          const fromAccount = 'POOJA INCOME A/C';
          const toAccount = accountSel?.ledgerName || 'CASH A/C';
  ...
  router.put('/:id', async (req, res) => {
    try {
      ...
      const pooja = await db('pooja').where({ id, temple_id: req.user.templeId }).first();
      if (!pooja) {
        return res.status(404).json({ error: 'Pooja entry not found' });
      }
      ...
      const bookingDate = p.bookingDate || p.fromDate;
      const entryDate = p.entryDate || pooja.entry_date || new Date().toISOString().slice(0, 10);
      let accountSel = null;
      try {
        accountSel = await resolveAccountLedgerName(req.user.templeId, p.paymentMode, p.accountId);
      } catch (e) {
      ...
      const updatedPooja = await db('pooja').where({ id }).first();
      ...
      try {
        ...
        if (hasJournal) {
          await db('journal_entries')
            ...
            .del();
          const amountNum = Number(p.amount || updatedPooja.amount || 0);
          if (!isNaN(amountNum) && amountNum > 0) {
            ...
            await db('journal_entries').insert({
              ...
              total_amount: amountNum,
              entry_type: 'pooja',
              description: `Pooja: ${updatedPooja.name} (${updatedPooja.receipt_number || id})`,
              reference_number: updatedPooja.receipt_number || 'POOJA-' + id,
              remarks: updatedPooja.remarks || null,
              reference_type: 'pooja',
      ...
      try {
        await logPoojaAction({
          ...
          userId: req.user.id,
          action: 'update',
          details: { before: pooja, after: updatedPooja },
        });
      } catch (logError) {
  ...
  router.get('/export', async (req, res) => {
    try {
      ...
      const headers = [
        'id,receipt_number,name,mobile_number,time,booking_date,entry_date,remarks,transfer_to_account,amount,created_at'
      ];
      ...
      const csv = rows.map(r => [
        ...
        r.mobile_number,
        r.time, 
        r.booking_date || r.from_date, 
        r.entry_date || r.created_at, 
  ...
  router.get('/stats/summary', async (req, res) => {
    try {
      ...
      if (from) query = query.andWhere('booking_date', '>=', from);
      if (to) query = query.andWhere('booking_date', '<=', to);
      
      const stats = await query
        .select(
          db.raw('COUNT(*) as total_entries'),
          db.raw('COUNT(DISTINCT name) as unique_people'),
          db.raw('AVG(DATEDIFF(to_date, from_date) + 1) as avg_duration_days')
        )
        .first();
