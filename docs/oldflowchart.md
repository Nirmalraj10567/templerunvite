...
flowchart TD
    ...
    D --> B
    C -->|Valid| E[Generate Reference Number]
    E --> F[Upload Photo (if any)]
    F --> G[Save to Database]
    G --> H{Success?}
    H -->|No| I[Show Error Message]
    H -->|Yes| J[Add Heirs (if any)]
    J --> K[Send Success Response]
    K --> L[Display Confirmation]
    
    subgraph "Reference Number Generation"
        M[Get Current Year]
        N[Query Last Reference for Year]
        O[Generate Next Sequential Number]
        P[Format: YYYY-NNNN]
    end
...
flowchart TD
    ...
    L --> M[Compression & Storage]
    M --> N[Save Event]
    N --> O[Save Image Metadata]
    O --> P[Update Calendar]
    ...
    E --> V[Confirm Deletion]
    V --> W{Confirmed?}
    W -->|No| X[Cancel Operation]
    W -->|Yes| Y[Delete Images]
    Y --> Z[Delete Event Record]
    Z --> AA[Update Calendar]
    AA --> BB[Confirm Deletion]
    
    F --> CC[Search & Filter]
    CC --> DD[Paginate Results]
    DD --> EE[Display Events]
    
    Q --> FF[Complete Process]
    BB --> FF
    EE --> FF
    X --> FF
```
...
flowchart TD
    ...
    C --> E[Select Pooja Type]
    D --> F[Mobile Submission]
    
    E --> G[Check Availability]
    F --> H[Basic Validation]
    
    G --> I{Available?}
    I -->|No| J[Suggest Alternatives]
    I -->|Yes| K[Fill Booking Form]
    
    H --> L{Valid?}
    L -->|No| M[Return Error]
    L -->|Yes| N[Check Conflicts]
    
    N --> O{Conflict?}
    O -->|Yes| P[Return Conflict]
    O -->|No| Q[Save as Pending]
    
    K --> R[Devotee Details]
    R --> S[Contact Information]
    S --> T[Date Range Selection]
    T --> U[Time Slot Selection]
    U --> V[Remarks/Notes]
    
    V --> W[Generate Receipt Number]
    W --> X{Direct Approval?}
    
    X -->|Yes| Y[Create Booking]
    X -->|No| Z[Submit for Approval]
    
    Y --> AA[Update Calendar]
    Z --> AB[Admin Review]
    
    AA --> AC[Log Creation]
    AB --> AD{Admin Decision}
    
    AD -->|Approve| AE[Approve Pooja]
    AD -->|Reject| AF[Reject with Reason]
    AD -->|Request Changes| AG[Request Modifications]
    
    AE --> AC
    AF --> AH[Notify Rejection]
    AG --> AI[Notify User]
    
    AC --> AJ[Send Confirmation]
    AH --> AK[Update Status]
    AI --> AL[User Updates]
    AL --> AB
    
    AJ --> AM[Complete Process]
    AK --> AM
    
    Q --> AN[Return Pending Status]
    J --> AO[End Process]
    M --> AO
    P --> AO
    AN --> AO
...
flowchart TD
    ...
    X --> Y[Tax Amount]
    X --> Z[Amount Paid]
    X --> AA[Outstanding Amount]
    
    AA --> AB[Account Transfer]
    AB --> AC[From Account]
    AB --> AD[To Account]
    
    AC --> AE[Upload Photo]
    AE --> AF[Validate Form Data]
    
    AF --> AG{Valid?}
    AG -->|No| AH[Show Validation Errors]
    AG -->|Yes| AI[Generate Reference Number]
    
    AI --> AJ[Save to Database]
    AJ --> AK[user_tax_registrations]
    AK --> AL[Log Creation]
    
    AL --> AM{Amount Paid > 0?}
    AM -->|Yes| AN[Create Journal Entry]
    AM -->|No| AO[Skip Journal]
    
    AN --> AP[Sync to Ledger]
    AP --> AQ[Return Success]
    AO --> AQ
    
    D --> AR[Filter & Search]
    AR --> AS[By Year]
    AR --> AT[By Name]
    AR --> AU[By Reference Number]
    
    E --> AV[Load Registration Data]
    AV --> AW[Edit Fields]
    AW --> AX[Validate Changes]
    AX --> AY[Update Database]
    AY --> AZ[Update Journal Entry]
    AZ --> BA[Log Update]
    BA --> BB[Return Success]
    
    F --> BC[Verify Exists]
    BC --> BD[Delete Journal Entry]
    BD --> BE[Delete Registration]
    BE --> BF[Log Deletion]
    BF --> BG[Return Success]
    
    G --> BH[Fetch Registration Data]
    BH --> BI[Load PDF Settings]
    BI --> BJ[Generate Tamil Receipt]
    BJ --> BK[Return PDF]
...
flowchart TD
    ...
    L -->|Yes| N[Set Temple ID]
    
    N --> O[Insert into properties]
    O --> P[Log Creation]
    P --> Q[Return 201 Success]
    
    D --> R[Query properties Table]
    R --> S[Filter by Temple ID]
    S --> T[Order by Updated At]
    T --> U[Return Property List]
    
    E --> V[Load Property Data]
    V --> W[Verify Ownership]
    W --> X{Authorized?}
    X -->|No| Y[Return 403]
    X -->|Yes| Z[Update Fields]
    Z --> AA[Update Database]
    AA --> AB[Return 200 Success]
    
    F --> AC[Verify Ownership]
    AC --> AD{Authorized?}
    AD -->|No| AE[Return 403]
    AD -->|Yes| AF[Delete Property]
    AF --> AG[Return 200 Success]
...
flowchart TD
    ...
    G -->|Food| H[Enter Food Details]
    G -->|Product| I[Enter Product Details]
    G -->|Money| J[Enter Amount Details]
    
    H --> K[Specify Number of People]
    I --> L[Product Name & Quantity]
    J --> M[Donation Amount]
    
    K --> N[Select Date Range]
    L --> N
    M --> N
    
    N --> O[Generate Receipt Number]
    O --> P[Validate Data]
    P --> Q{Valid?}
    ...
    T -->|No| U[Direct Approval]
    T -->|Yes| V[Submit for Approval]
    
    U --> W[Generate Receipt]
    V --> X[Admin Review]
    
    X --> Y{Admin Decision}
    Y -->|Approve| Z[Approve Request]
    Y -->|Reject| AA[Reject with Reason]
    Y -->|Request Changes| AB[Request Modifications]
    
    Z --> W
    AA --> AC[Send Rejection Notice]
    AB --> AD[Notify User]
    
    W --> AE[Update Calendar]
    AC --> AF[Log Rejection]
    AD --> AE
    
    AE --> AG[Send Confirmation]
    AF --> AH[Update Status]
    AG --> AI[Complete Process]
    AH --> AI
    R --> E
```
